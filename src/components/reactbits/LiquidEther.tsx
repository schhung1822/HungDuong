...
type RenderTargetMap = Record<string, THREE.WebGLRenderTarget | null>;
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    function makePaletteTexture(stops: string[]): THREE.DataTexture {
      const palette = stops.length > 0 ? (stops.length === 1 ? [stops[0], stops[0]] : stops) : ["#ffffff", "#ffffff"];
    ...
    const transparentBg = new THREE.Vector4(0, 0, 0, 0);
    ...
    class MouseClass {
      ...
      private onMouseMoveHandler = this.onDocumentMouseMove.bind(this);
      private onTouchStartHandler = this.onDocumentTouchStart.bind(this);
      private onTouchMoveHandler = this.onDocumentTouchMove.bind(this);
      private onTouchEndHandler = this.onTouchEnd.bind(this);
      private onDocumentLeaveHandler = this.onDocumentLeave.bind(this);
      ...
      init(container: HTMLElement) {
        ...
        this.listenerTarget.addEventListener("mousemove", this.onMouseMoveHandler);
        this.listenerTarget.addEventListener("touchstart", this.onTouchStartHandler, { passive: true });
        this.listenerTarget.addEventListener("touchmove", this.onTouchMoveHandler, { passive: true });
        this.listenerTarget.addEventListener("touchend", this.onTouchEndHandler);
        this.docTarget.addEventListener("mouseleave", this.onDocumentLeaveHandler);
      }

      dispose() {
        this.listenerTarget?.removeEventListener("mousemove", this.onMouseMoveHandler);
        this.listenerTarget?.removeEventListener("touchstart", this.onTouchStartHandler);
        this.listenerTarget?.removeEventListener("touchmove", this.onTouchMoveHandler);
        this.listenerTarget?.removeEventListener("touchend", this.onTouchEndHandler);
        this.docTarget?.removeEventListener("mouseleave", this.onDocumentLeaveHandler);
    ...
    class AutoDriver {
      ...
      activationTime = 0;
      margin = 0.2;
      private tempDir = new THREE.Vector2();
      ...
      update() {
        ...
        const dir = this.tempDir.subVectors(this.target, this.current);
    ...
    class ShaderPass {
      ...
      constructor(props: Record<string, unknown>) {
        this.props = props || {};
        const material = this.props.material as { uniforms?: Uniforms } | undefined;
      ...
      init() {
        ...
        const materialProps = this.props.material as THREE.ShaderMaterialParameters | undefined;
    ...
    class Advection extends ShaderPass {
      ...
      update(props?: { dt?: number; isBounce?: boolean; BFECC?: boolean }) {
        ...
        if (typeof props?.isBounce === "boolean") this.line.visible = props.isBounce;
        if (typeof props?.BFECC === "boolean") this.uniforms.isBFECC.value = props.BFECC;
        super.update();
      }
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props?: { mouse_force?: number; cellScale?: THREE.Vector2; cursor_size?: number }) {
        ...
        uniforms.force.value.set((Mouse.diff.x / 2) * (props?.mouse_force || 0), (Mouse.diff.y / 2) * (props?.mouse_force || 0));
    ...
    class Viscous extends ShaderPass {
      ...
      update(props?: { viscous?: number; iterations?: number; dt?: number }) {
        ...
        if (typeof props?.viscous === "number") this.uniforms.v.value = props.viscous;
        let fboOut: THREE.WebGLRenderTarget | null = null;
        for (let i = 0; i < (props?.iterations || 0); i++) {
          const fboIn = this.props[i % 2 === 0 ? "output0" : "output1"] as THREE.WebGLRenderTarget;
          fboOut = this.props[i % 2 === 0 ? "output1" : "output0"] as THREE.WebGLRenderTarget;
    ...
    class Divergence extends ShaderPass {
      ...
      update(props?: { vel?: THREE.WebGLRenderTarget | null }) {
        if (this.uniforms && props?.vel) this.uniforms.velocity.value = props.vel.texture;
        super.update();
      }
    }
    ...
    class Poisson extends ShaderPass {
      ...
      update(props?: { iterations?: number }) {
        ...
        for (let i = 0; i < (props?.iterations || 0); i++) {
          const fboIn = this.props[i % 2 === 0 ? "output0" : "output1"] as THREE.WebGLRenderTarget;
          fboOut = this.props[i % 2 === 0 ? "output1" : "output0"] as THREE.WebGLRenderTarget;
          this.uniforms.pressure.value = fboIn.texture;
          this.props.output = fboOut;
          super.update();
        }
        return fboOut;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      running = false;
      private loopHandler = this.loop.bind(this);
